...
export class ResizableDirective implements OnInit {
  ...
  constructor(private el: ElementRef) {
    ...
    const newWidth = (wid) => {
      const newWidthResize = Math.max(this.resizableMinWidth, wid);
      el.nativeElement.style.width = (newWidthResize) + 'px';
    };
    ...
    const dragMoveG = (evt) => {
      ...
      const newWidthResize = Math.max(this.resizableMinWidth, (evt.clientX - el.nativeElement.offsetLeft)) + 'px';
  ...
  ngOnInit(): void {
    this.el.nativeElement.style['border-right'] = this.resizableGrabWidth + 'px double #dadada';
  }
